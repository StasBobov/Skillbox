...
class Trade_statistic(multiprocessing.Process):
    tickers_data = {}

    def __init__(self, name, collector, file_path, *args, **qwargs):
    ...
    def calc(self, ticker_list):
        ...
        res_dict[deal_inf[0]] = volatility
        self.collector.put(res_dict)
...
def main():
    ...
    statistic = [Trade_statistic(name=file, collector=collector, file_path=os.path.join(current_path, file)) for file in os.listdir(current_path)]
    ...
    while not collector.empty():
        ...
        for key, value in data.items():
            Trade_statistic.tickers_data[key] = value
